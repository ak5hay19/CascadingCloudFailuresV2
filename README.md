...
1. **Dynamic graph topology** — the graph structure changes every timestep, reflecting which machines are actually co-located in the same cluster/collection during each 5-minute window. Most ST-GNN papers use static edges.
2. **MC Dropout uncertainty quantification** — instead of a hard yes/no prediction, the model outputs confidence bounds ("87% failure probability, ±6% uncertainty").
3. **Edge contagion scoring** — identifies which specific connections between machines carry the most failure signal, telling operators which dependencies to sever during incidents.
...
Input: 6 consecutive DYNAMIC graph snapshots (30 min history)
       Each snapshot: ~4,900 nodes × 13 features + PER-WINDOW edges

                    ┌──────────────────────────┐
  Snapshot t=1 ──> │  2-layer GraphSAGE         │ ──> h1  [N, 48]
  (edges_t1)  ──>  │  (samples 15 neighbors/node │
  Snapshot t=2 ──> │   per layer)                │ ──> h2  [N, 48]
  (edges_t2)  ──>  │                             │
  ...              │  edges CHANGE per timestep   │
  Snapshot t=6 ──> │  + residual connections      │ ──> h6  [N, 48]
  (edges_t6)  ──>  │                              │
                    └──────────────────────────┘
                                │
...
python check_data.py     # (optional) verify
...
- **13 features per machine per window:** CPU mean/std/max, memory mean/std/max, assigned memory, task count, failure count/rate, priority mean, scheduling diversity, event count
...
- Mini-batch gradient accumulation (batch=8)
