...
def build_snapshot(features, labels, m2i, adj, ckpt, tw_slice, edge_builder):
    ...
    # Explicitly drop leaky columns to match the 25-feature training setup
    leaky_cols = {"failed_sum", "failed_mean", "instance_events_type_count"}
    feat_cols = [c for c in features.columns if c not in ("machine_id", "time_window") and c not in leaky_cols]
    
    fm, fs = ckpt["feat_mean"], ckpt["feat_std"]
    
    # Verify shape matches (safeguard)
    if len(feat_cols) != len(fm):
        print(f"Warning: Feature count mismatch. Expected {len(fm)} from model, got {len(feat_cols)} from data.")
...
def plot_embedding_tsne(model, features, labels, m2i, adj, ckpt,
    ...
    # Exclude leaky columns to ensure shape matches (25 features)
    leaky_cols = {"failed_sum", "failed_mean", "instance_events_type_count"}
    feat_cols = [c for c in features.columns if c not in ("machine_id", "time_window") and c not in leaky_cols]
...
def edge_contagion_scoring(model, features, labels, m2i, adj, ckpt,
    ...
    max_val = edge_importance.max()
    if max_val > 0:
        edge_importance = edge_importance / max_val
    else:
        edge_importance += 1e-8  # Visual fallback if absolutely zero
    ...
    if w_max > w_min:
        # Min-max scaling raw scores to [1.0, 5.0] width range
        ew = [1.0 + 4.0 * (w - w_min) / (w_max - w_min) for w in weights]
    else:
        # Visual fallback if scores are identical
        ew = [1.0 for _ in weights]
    ...
    for _, e, score in top_edges[:10]:
        ...
        print(f"    {e[0]}({sf}) -> {e[1]}({df_label})  score={score:.2e}")
...
def main():
    ...
    # Read dynamic config threshold
    with open("config.yaml", "r") as f:
        config = yaml.safe_load(f)
    eval_threshold = config.get("training", {}).get("eval_threshold", 0.1)

    data = np.load(results_path)
    labels_arr, probs = data["labels"], data["probs"]
    # Dynamically recompute predictions rather than taking hardcoded model outputs
    preds = (probs >= eval_threshold).astype(int)
    
    print(f"\nUsing eval_threshold={eval_threshold} from config.yaml")
